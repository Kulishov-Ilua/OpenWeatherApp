...
class CityWeatherRepositoryImpl @Inject constructor(
    ...
    override fun getCityWeather(): Flow<List<WeatherForecastResponse>> =
        cityWeatherDao.getCityWeather()
            .map { entities -> entities.map { entitity -> CityWeatherMapper.toDomain(entitity) } }

    override fun getCityWeatherByName(name: String): Flow<List<WeatherForecastResponceWithDateTime>> =
        cityWeatherDao.getCityWeatherByName(name).map { entities ->
            entities.map { entitity ->
                CityWeatherMapper.toDomainWithTime(entitity)
            }
        }
